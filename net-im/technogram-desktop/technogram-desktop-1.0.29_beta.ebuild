...
HOMEPAGE="https://git.technology45.ru/Messenger/tdesktop"
SRC_URI="(
...
EGIT_REPO_URI="git@git.technology45.ru:Messenger/tdesktop.git"
EGIT_BRANCH="api65"
#EGIT_COMMIT="v${PV}"
EGIT_COMMIT="${PV}"
...
src_compile(){
	...
	emake || die 'Make failed'
	chrpath --delete Technogram
}

src_install(){
	newbin ${S}/out/Release/Technogram ${PN}
	insopts -m644
	for icon_size in 16 32 48 64 128 256 512; do
		newicon -s ${icon_size} ${S}/Telegram/Resources/art/icon${icon_size}.png ${PN}.png
	done
	make_desktop_entry "${PN} -- %u" "Technogram" ${PN} "Network;" "MimeType=x-scheme-handler/tech;"
}
